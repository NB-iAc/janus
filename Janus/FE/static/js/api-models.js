class ApiModels {
    ...
    static buildingFromDTO(dto) {
        return {
            ...
            total_floors: dto.total_floors || dto.totalFloors || 0,
    ...
    static mapObjectToCreateDTO(mapObject) {
        ...
        return {
            ...
            category,
            contactDetails: `${mapObject.metadata?.owner || ""} | ${mapObject.metadata?.email || ""} | ${mapObject.metadata?.phone || ""}`,
...
class ApiClient {
    async getRoomsByBuilding(buildingId) {
        const response = await this.fetchApi(`/get_rooms?building_id=${buildingId}`);
    ...
    async requestEditPermission(roomId) {
        const response = await this.fetchApi("/request_room_access", {
            method: "POST",
        ...
        if (response && response.success) {
            ...
            throw new Error("Failed to request edit permission");
        }
    }
    async getRoomDetails(roomId) {
        const response = await this.fetchApi(`/get_room_details?room_id=${roomId}`);
    ...
    async updateRoomDetails(roomId, roomData) {
        const response = await this.fetchApi(`/update_room/${roomId}`, {
            method: "PUT",
